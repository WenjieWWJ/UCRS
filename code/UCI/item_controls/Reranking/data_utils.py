...
def map_features(user_feature_path, item_feature_path, return_feat_map=False):
    ...
    for itemID in item_feature:
        ...
        for f in feat:
            mapped_feat.append(features[f])
#         mapped_item_feature[itemID] = [mapped_feat[0:1], item_feature[itemID][1][0:1]]
        mapped_item_feature[itemID] = [mapped_feat, item_feature[itemID][1]]

    item_map_dict = {}
    i = 0
    for key in item_feature:
        if key not in item_map_dict:
            item_map_dict[key] = i
            i += 1
    ...
    for key in user_feature:
        if key not in user_map_dict:
            user_map_dict[key] = i
            i += 1
        if return_feat_map:
            return mapped_user_feature, mapped_item_feature, len(features), user_map_dict, item_map_dict, features
    return mapped_user_feature, mapped_item_feature, len(features), user_map_dict, item_map_dict
...
class FMData(data.Dataset):
    ...
    def __init__(self, train_path, user_feature, item_feature, loss_type, user_map_dict, item_map_dict, negative_num=1):
        ...
        for pair in self.train_list:
            ...
            self.train_dict[userID].append(item_map_dict[itemID])  ## map into 0 ~ item_num
